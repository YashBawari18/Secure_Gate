...
export default function Passes() {
  ...
  const load = async () => {
    ...
    } catch { toast.error(t('passes.failLoad', 'Failed to load passes')); }
  ...
  const loadQR = async (id, name) => {
    ...
    } catch { toast.error(t('passes.failLoadQr', 'Failed to load QR')); }
  };
      ...
      {qrModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}
          ...
          <div style={{ background: 'var(--bg2)', borderRadius: 16, padding: 28, textAlign: 'center', minWidth: 200 }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 14 }}>{t('passes.qrPass', 'QR Pass —')} {qrModal.name}</div>
            <img src={qrModal.img} alt="QR Code" style={{ width: 180, height: 180, borderRadius: 10 }} />
            <div style={{ marginTop: 14 }}><button className="btn btn-ghost btn-sm" onClick={() => setQrModal(null)}>{t('passes.close', 'Close')}</button></div>
          </div>
        </div>
      )}
            ...
            <tbody>
              {passes.length === 0 ? (
                ...
                <tr key={p._id}>
                  ...
                  <td style={{ textTransform: 'capitalize' }}>{t(`admin.${p.purpose}`, p.purpose)}</td>
                  <td>
                  ...
                  <td style={{ fontFamily: 'var(--mono)', letterSpacing: 2, fontWeight: 600, color: 'var(--pri)' }}>
                    {p.otpUsed ? <span className="badge badge-gray">{t('passes.used', 'Used')}</span> : (p.otp || '—')}
                  </td>
                  <td style={{ fontSize: 12, color: 'var(--tx3)' }}>
                    {p.validDate ? new Date(p.validDate).toLocaleDateString() : t('passes.today', 'Today')}
                  </td>
                  <td>
                    {p.entryMethod === 'qr' ? (
                      <button className="btn btn-ghost btn-sm" onClick={() => loadQR(p._id, p.name)}>{t('passes.viewQr', 'View QR')}</button>
