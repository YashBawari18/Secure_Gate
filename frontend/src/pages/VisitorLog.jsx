...
export default function VisitorLog() {
  ...
  const handleExit = async (id) => {
    try { await visitorsAPI.exit(id); toast.success('Exit recorded'); load(); }
        ...
        {loading ? (
          ...
          <div className="table-responsive">
            <table className="data-table">
              <thead>
                <tr><th>Visitor</th><th>Flat</th><th>Purpose</th><th>Entry</th><th>Exit</th><th>Method</th><th>Status</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {visitors.length === 0 ? (
                  <tr><td colSpan={8} style={{ textAlign:'center', color:'var(--tx3)', padding:32 }}>No visitors found</td></tr>
                ) : visitors.map(v => (
                  <tr key={v._id}>
                    <td>
                      <div style={{ fontWeight:500 }}>{v.name}</div>
                      <div style={{ fontSize:10, color:'var(--tx3)', fontFamily:'var(--mono)' }}>{v.phone}</div>
                      {v.isSuspicious && <span className="badge badge-red" style={{ marginTop:3, fontSize:9 }}>Suspicious</span>}
                    </td>
                    <td style={{ fontFamily:'var(--mono)', fontWeight:500 }}>{v.flatNumber||'—'}</td>
                    <td style={{ textTransform:'capitalize', color:'var(--tx2)' }}>{v.purpose}</td>
                    <td style={{ fontFamily:'var(--mono)', fontSize:12 }}>{fmt(v.entryTime)}</td>
                    <td style={{ fontFamily:'var(--mono)', fontSize:12 }}>{fmt(v.exitTime)}</td>
                    <td><span className={`badge badge-${METHOD_COLOR[v.entryMethod]||'gray'}`} style={{ textTransform:'uppercase', fontSize:9 }}>{v.entryMethod}</span></td>
                    <td><span className={`badge badge-${STATUS_COLOR[v.status]||'gray'}`} style={{ textTransform:'capitalize' }}>{v.status}</span></td>
                    <td>
                      <div style={{ display:'flex', gap:4 }}>
                        {v.status === 'approved' && !v.exitTime && (
                          <button className="btn btn-ghost btn-sm" onClick={()=>handleExit(v._id)}>Exit</button>
                        )}
                        {!v.isWatchlisted && (v.isSuspicious || v.status==='denied' || v.flagCount>0) && (
                          <button className="btn btn-danger btn-sm" onClick={()=>handleWatch(v._id)}>Watch</button>
                        )}
                      </div>
                    </td>
                  </tr>
